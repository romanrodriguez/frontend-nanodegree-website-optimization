...
function logAverageFrame(e) {
    ...
    console.log("Average time to generate last 60 frames: " + r / 60 + "ms")
}

function updatePositions() {
    frame++, window.performance.mark("mark_start_frame");
    for (var e = document.querySelectorAll(".mover"), a = document.body.scrollTop, r = 0; r < e.length; r++) {
        var n = Math.sin(a / 1250 + r % 5);
        e[r].style.left = e[r].basicLeft + 100 * n + "px"
    }
    if (window.performance.mark("mark_end_frame"), window.performance.measure("measure_frame_duration", "mark_start_frame", "mark_end_frame"), frame % 60 === 0) {
...
window.addEventListener("scroll", updatePositions), document.addEventListener("DOMContentLoaded", function() {
    for (var e = 8, a = 256, r = document.querySelector("#movingPizzas1"), e = 8, a = 256, n = 30; n--;) {
        var i = document.createElement("img");
        i.className = "mover", i.src = "images/pizza.png", i.basicLeft = n % e * a, i.style.top = Math.floor(n / e) * a + "px", r.appendChild(i)
    }
    updatePositions()
});